...
<div class='billtable'>
	<div class='billtable-layout  billtable-header'>
		<h1>ВЫПИСКА ЛИЦЕВОГО СЧЕТА за <?php setlocale(LC_TIME, 'ru_RU.UTF8'); echo strftime("%B %Y",strtotime($Form['period']));  ?></h1>
		<div class='billtable-layout billtable-info'>
			<p>Информация о собственнике:</p>
			<p>ФИО:<?= $Form['account']['employer'];?>;&nbsp;&nbsp;ЛС:&nbsp;<?= $Form['account']['accNumber'];?></p>
			<p>Адрес:<?= $Form['account']['location'];?></p>
			<p>Общая площадь:&nbsp;<?= $Form['account']['square'];?>м<sup>2</sup>;&nbsp;&nbsp;Зарегестрированно:&nbsp;<?= $Form['account']['registered'];?>чел.</p>
		</div>
	...
	<div class='billtable-layout billtable-qr'>
		<img src="https://api.qrserver.com/v1/create-qr-code/?size=150x150&data=<?= $Form['account']['qr']; ?>" alt="" />
	</div>
	<div class='billtable-table-layout'>
							...
							if(in_array($srv,$usrv)){
								...
								echo "<tr>";
									...
									if(!$ix){
										...
										echo "<td>".(isset($info['usedvolume']) ? $info['usedvolume'] : 0)."</td>";
									}
									else{
		...
		<div class='billtable-foot-layout'>
			<table class='billtable-table' border='1'>
				...
				<tr>
					<td>Долг на <?= strftime("01.%m.%Y",strtotime($Form['period'])); ?>:</td>
					<td><?= number_format($allsaldo, 2, ',', ' '); ?></td>
				</tr>
				<tr>
					<td>Начислено:</td>
					<td><?= number_format($allitog, 2, ',', ' '); ?></td>
				</tr>
				<tr>
					<td>Оплачено:</td>
					<td><?= number_format($allpay, 2, ',', ' ');  ?></td>
				</tr>
				<tr>
					<td>К оплате:</td>
					<td><?= number_format(($allsaldo + $allitog - $allpay), 2, ',', ' '); ?></td>
				</tr>	
			</table>
		</div>
		<div class='billtable-foot-layout'>
