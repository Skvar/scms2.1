...
<form class='form-default-panel edit-file-panel' method='POST'  id='fileEditForm' name='fileEditForm' enctype='multipart/form-data' action='./templates/base/forms/EditFileForm.php'>
	<input type='hidden' id='file-owner' value='<?= $Form['file-owner']; ?>'>
	<input type='hidden' id='file-link' value=''>
	<fieldset <?= ((isset($Form['file-id']) && $Form['file-id']>0) ? "style='display:none;'":"");  ?>>
	<legend>Выбрать файл...</legend>
	<div class='file-upload'>
		...
		<input type='text' class='file-name' readonly='true' id='file-name' value='<?= $Form['file-name']; ?>'>
	</div>
	<div id='file-progressbar' class='file-progressbar'><div id='file-bar' class='file-bar'></div></div>
	</fieldset>
	<fieldset>
	<legend>Описание файла</legend>
<div>
		<textarea id='file-description' style='height:100px;'><?= $Form['file-description']; ?></textarea>
</div>
<div>
		<table>
			<tr>
				<td>
					<label>Дата публикации:<input type='date' id='file-date' value='<?= $Form['file-date']; ?>'></label>	
				</td>
				<td>
...
<hr>
<button type='button' id='save-file-button' class='normal-button' onclick="WriteFormData('<?= $Form['handler']; ?>','fileEditForm','<?= $Form['arg']; ?>');" <?= ((isset($Form['file-id']) && $Form['file-id']>0) ? "":"disabled");  ?>>
