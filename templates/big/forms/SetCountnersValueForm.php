	 ...
	 <div class='input-panel'>
		 ...
		 <table>
			 <tr>
				 ...
				 <th  width='15%'>Предыдущие показания<br><?php echo $Counters[0]['values'][0]['date']; ?></th>
				 <th  width='15%'>Текущие показания<br><?php echo date("d.m.Y"); ?></th>
...
//Сбор информации----------------------------------------------------------
	...
	SendRequest('<?php echo $Form['handler']; ?>','ajax=1<?php echo $Form['arg']; ?>&data='+datastr,function(req){
		if(req.responseText.length > 0){
			try{
	        	 obj = JSON.parse(req.responseText);
	        	 if(obj['result'] == true){	
	        	 	out  = document.getElementById('<?php echo $Form['out']; ?>');
	        	 	if(out){					
...
function onInput(inp,id) {
  if (inp.value.indexOf(".") != '-1') {
    inp.value=inp.value.substring(0, inp.value.indexOf(".") + 3);
  }
  ClosePopups();
  ...
  var maxResource = <?php echo coreSettings('maxResourceValue'); ?>;
