...
<div class='files-form'> 
	...
	<p onclick='ToggleOpenBlock("files-list<?php echo $Files['index']; ?>")'><?php echo isset($Files['header']) ? $Files['header']: "Список приложенных файлов:"; ?></p>
	<div class='files-list files-list-list<?php echo $Files['show'] ? ' files-list-open':''; ?>' id='files-list<?php echo $Files['index']; ?>'>
	<?php
		$user = GetCurrentUser();
		foreach($Files['list'] as $key=>$val){		
			...
			switch($val['type']){
				...
				case fileDoc:
					...
					echo "<a href='https://ваш-управдом-плюс.рф/".$val['link']."' target='_blank'>(<i>Скачать</i>)</a>";
				break;
				default:
